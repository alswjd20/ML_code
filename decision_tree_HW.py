...
dataset = pd.read_csv("C:/Users/minje/Desktop/week3/W03-zoo.csv",
...
def entropy(target_col):
    ...
    for i in range(0, len(elements)):
        prob = counts[i] / len(target_col)
        entropy_t += -prob * math.log2(prob)
...
def InfoGain(data, split_attribute_name, target_name="class"):
    ...
    for i in range(0, len(vals)):
        weighted_entropy += (counts[i] / len(data)) * entropy(data.where(data[split_attribute_name] == vals[i]).dropna())
