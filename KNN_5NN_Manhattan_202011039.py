# -*- coding: utf-8 -*-
"""
Created on Mon Oct 10 14:01:34 2022
...
"5NN-Manhattan"
nearest_5_manhatt = list()

def KNN5_Manhattan_Classify(data, tf_draft, new_data):
    nearest_5_manhatt.clear()
    manhattan_distance(data, new_data)      #거리 계산
    manhatt_dist.sort(key = lambda x:x[1])   #거리순으로 정렬
    nearest_5_manhatt.append(manhatt_dist[0])
    nearest_5_manhatt.append(manhatt_dist[1])
    nearest_5_manhatt.append(manhatt_dist[2])
    nearest_5_manhatt.append(manhatt_dist[3])
    nearest_5_manhatt.append(manhatt_dist[4]) # 가장 가까운 5개의 data 리스트 추출
    print(nearest_5_manhatt)
    ...
    for i in range(0,5):        #가장 가까운 5개 data의 라벨을 확인
        if(tf_draft[nearest_5_manhatt[i][0]] == 'Yes'):     #라벨이 Yes에 해당되면
            cnt_yes += 1
...
KNN5_Manhattan_Classify(data, tf_draft, [6.75, 3])
KNN5_Manhattan_Classify(data, tf_draft, [5.34, 6.0])
KNN5_Manhattan_Classify(data, tf_draft, [4.67, 8.4])
KNN5_Manhattan_Classify(data, tf_draft, [7.0, 7.0])
KNN5_Manhattan_Classify(data, tf_draft, [7.8, 5.4])
