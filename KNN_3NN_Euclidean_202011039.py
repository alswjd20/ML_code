# -*- coding: utf-8 -*-
"""
Created on Sun Oct  9 16:41:00 2022
...
"3NN-Euclidean"
nearest_3_euclid = list()

def KNN3_Euclidean_Classify(data, tf_draft, new_data):
    nearest_3_euclid.clear()
    euclidean_distance(data, new_data)      #거리 계산
    euclid_dist.sort(key = lambda x:x[1])   #거리순으로 정렬
    nearest_3_euclid.append(euclid_dist[0])
    nearest_3_euclid.append(euclid_dist[1])
    nearest_3_euclid.append(euclid_dist[2]) # 가장 가까운 3개의 data 리스트 추출
    print(nearest_3_euclid)
    ...
    for i in range(0,3):            #가장 가까운 3개 data의 라벨을 확인
        if(tf_draft[nearest_3_euclid[i][0]] == 'Yes'):   #라벨이 Yes에 해당되면
            cnt_yes += 1                        
...
KNN3_Euclidean_Classify(data, tf_draft, [6.75, 3])
KNN3_Euclidean_Classify(data, tf_draft, [5.34, 6.0])
KNN3_Euclidean_Classify(data, tf_draft, [4.67, 8.4])
KNN3_Euclidean_Classify(data, tf_draft, [7.0, 7.0])
KNN3_Euclidean_Classify(data, tf_draft, [7.8, 5.4])
